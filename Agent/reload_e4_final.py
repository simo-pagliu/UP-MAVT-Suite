#!/usr/bin/env python3
"""
Reload E4 session with CORRECTED data (the (1) files)
"""
import csv
...
# CSV file paths (CORRECT ORIGINAL versions without (1))
...
# Load QI data (CORRECTED - without pre-computed values)
...
try:
    with open(QI_CSV, 'r') as f:
        ...
        for row in reader:
            ...
            # VALUE field is empty in corrected file - ignore it
            ...
            # Don't set values from CSV - they will be computed from ranking
            qi_data[criterion_name]['confidences'][str(rank)] = confidence
    
    print(f"✓ Loaded QI data for {len(qi_data)} criteria")
    print(f"  Note: VALUES not pre-filled (will be computed from ranking)")
except FileNotFoundError as e:
    print(f"ERROR: {e}")
    exit(1)
...
print("\n💾 Updating E4 session with CORRECTED data...")
...
# Add Qualitative Indicators (without pre-computed values)
...
if result.modified_count > 0 or result.matched_count > 0:
    ...
    # Show sample VF to verify correctness
    sample_vf = vf_criteria.get('Construction Complexity', {})
    if sample_vf.get('points'):
        print(f"\n📊 Sample VF (Construction Complexity):")
        for point in sample_vf['points'][:3]:
            print(f"   x={point['x']}, y={point['y']}")
    
    print(f"\n🎉 E4 session reloaded with CORRECTED data!")
else:
